...
describe('getUsersCards should return user cards', () => {
    ...
    // WordCard.find.mockImplementation(() => {return []});
    // WordCard.find = jest.fn();
    it('should return user cards', async () => {
        ...
        // expect().toEqual(datum.output);
    });
    it('should handle server error', async () => {
    WordCard.find.mockImplementation(() => {throw new Error('dummy error')});
        await getUsersCards(req, {json}, next);
    ...
    it('should handle null user cards', async () => {
        WordCard.find.mockResolvedValue(null);
            await getUsersCards(req, {json}, next);
            expect(next).toHaveBeenCalledWith(new HttpError(`a user with id 0 can not be found`, 404));
        })  
    // throw error
    // userCards = null
    // 
